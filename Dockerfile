...
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
    apt-get -y -q install iptables gcc make wget tar unzip dhcpcd5 iproute2 curl psmisc iputils-ping mtr procps net-tools iftop mtr tmux figlet build-essential && \
    apt-get clean && \
